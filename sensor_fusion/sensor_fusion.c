...
void initSensorFusionGlobals(SensorFusionGlobals* sfg, StatusSubsystem* pStatusSubsystem,
                             ControlSubsystem* pControlSubsystem) {
	sfg->iFlags =  // all of the following defines are either 0x0000 or a 1-bit value (2, 4, 8 ...) and are defined in
	               // build.h
	    F_USING_ACCEL | F_USING_MAG | F_USING_GYRO | F_USING_PRESSURE | F_USING_TEMPERATURE |
	...
	sfg->clearFIFOs = clearFIFOs;      // function to clear FIFO flags    sfg->applyPerturbation = ApplyPerturbation; //
	                                   // function used for step function testing
...
/// This function is normally involved via the "sfg." global pointer.
int8_t installSensor(SensorFusionGlobals* sfg,        ///< top level fusion structure
...
int8_t initializeSensors(SensorFusionGlobals* sfg) {
	...
	for (pSensor = sfg->pSensors; pSensor != NULL; pSensor = pSensor->next) {
		...
		if (status == 0)
			status = s;  // will return 1st error flag, but try all sensors
	}
	return (status);
}
...
void processAccelData(SensorFusionGlobals* sfg) {
	...
	fUpdateAccelBuffer(&(sfg->AccelCal), &(sfg->AccelBuffer), &(sfg->Accel),
...
void processMagData(SensorFusionGlobals* sfg) {
	...
	fRunMagCalibration(&(sfg->MagCal), &(sfg->MagBuffer), &(sfg->Mag), sfg->loopcounter);
...
/// This function is normally involved via the "sfg." global pointer.
int8_t readSensors(SensorFusionGlobals* sfg,   ///< pointer to global sensor fusion data structure
...
#if F_USING_ACCEL
	if (sfg->Accel.isEnabled)
		processAccelData(sfg);
#endif

#if F_USING_MAG
	if (sfg->Mag.isEnabled)
		processMagData(sfg);
#endif

#if F_USING_GYRO
	if (sfg->Gyro.isEnabled)
		processGyroData(sfg);
#endif
	return;
}
...
void runFusion(SensorFusionGlobals* sfg) {
	...
	fFuseSensors(pSV_1DOF_P_BASIC, pSV_3DOF_G_BASIC, pSV_3DOF_B_BASIC, pSV_3DOF_Y_BASIC, pSV_6DOF_GB_BASIC,
	             pSV_6DOF_GY_KALMAN, pSV_9DOF_GBY_KALMAN, pAccel, pMag, pGyro, pPressure, pMagCal);
	clearFIFOs(sfg);
}
...
void initializeFusionEngine(SensorFusionGlobals* sfg) {
	...
	if (sfg->iFlags & F_3DOF_B_BASIC)
		pComm->DefaultQuaternionPacketType = Q3M;
	if (sfg->iFlags & F_3DOF_Y_BASIC)
		pComm->DefaultQuaternionPacketType = Q3G;
	if (sfg->iFlags & F_6DOF_GB_BASIC)
		pComm->DefaultQuaternionPacketType = Q6MA;
	if (sfg->iFlags & F_6DOF_GY_KALMAN)
		pComm->DefaultQuaternionPacketType = Q6AG;
	if (sfg->iFlags & F_9DOF_GBY_KALMAN)
		pComm->DefaultQuaternionPacketType = Q9;
