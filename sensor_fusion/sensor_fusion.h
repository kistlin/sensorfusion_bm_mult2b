...
typedef int8_t(installSensor_t)(struct SensorFusionGlobals* sfg,  ///< Global data structure pointer
...
#define SPI_ADDR \
	0x00  // Use SPI_ADDR as the address parameter to the installSensor function for SPI-based sensors.
...
/// \brief An instance of PhysicalSensor structure type should be allocated for each physical sensors (combo devices =
/// 1)
///
...
void initSensorFusionGlobals(SensorFusionGlobals* sfg,                   ///< Global data structure pointer
...
void conditionSensorReadings(SensorFusionGlobals* sfg  ///< Global data structure pointer
);
void clearFIFOs(SensorFusionGlobals* sfg  ///< Global data structure pointer
);
runFusion_t runFusion;
readSensors_t readSensors;
void zeroArray(struct StatusSubsystem* pStatus,  ///< Status subsystem pointer
...
void conditionSample(int16_t sample[3]  ///< 16-bit register value from triaxial sensor read
);
...
void addToFifo(union FifoSensor* sensor,  ///< pointer to structure of type AccelSensor, MagSensor or GyroSensor
...
void ApplyAccelHAL(struct AccelSensor* Accel  ///< pointer to accelerometer logical sensor
);
/// \brief Apply the magnetometer Hardware Abstraction Layer
void ApplyMagHAL(struct MagSensor* Mag  ///< pointer to magnetometer logical sensor
);
/// \brief Apply the gyroscope Hardware Abstraction Layer
void ApplyGyroHAL(struct GyroSensor* Gyro  ///< pointer to gyroscope logical sensor
);
